...
export function EnhancedLiveView({ devices, selectedDevice, onDeviceSelect }: EnhancedLiveViewProps) {
  ...
  const startStream = async (device: Device) => {
    console.log(`[startStream] Starting for ${device.name} (${device.id})`)

    // Check if already streaming
    const currentState = streamStates.get(device.id)
    if (currentState?.status === 'connecting' || currentState?.status === 'connected') {
      console.log(`[startStream] Already ${currentState.status} for ${device.name}`)
      return
    }

    // Build URL immediately with proper format
    const streamId = device.id.includes('_hls') ? device.id : `${device.id}_hls`
    const hlsUrl = `http://localhost:3001/hls/${streamId}/playlist.m3u8`

    console.log(`[startStream] HLS URL for ${device.name}: ${hlsUrl}`)

    // CRITICAL FIX: Set URL immediately to avoid race condition
    setStreamStates(prev => {
      const newMap = new Map(prev)
      newMap.set(device.id, {
        deviceId: device.id,
        url: hlsUrl, // SET URL HERE IMMEDIATELY
        status: 'connecting',
        audioEnabled: false,
        recording: false
      })
    ...
    try {
      ...
      // Stream started successfully - update status only
      setStreamStates(prev => {
        const newMap = new Map(prev)
        newMap.set(device.id, {
          deviceId: device.id,
          url: hlsUrl, // KEEP THE URL
          status: 'connected',
          audioEnabled: false,
          recording: false
        })
      ...
      console.error(`[startStream] Failed for ${device.name}:`, err)
      setStreamStates(prev => {
        const newMap = new Map(prev)
        newMap.set(device.id, {
          deviceId: device.id,
          url: '', // Clear URL on error
          status: 'error',
          error: err.message,
          audioEnabled: false,
          recording: false
      ...
      toast.error(`Failed: ${device.name}`)
    }
  }
  ...
  useEffect(() => {
    const startStreamsSequentially = async () => {
    ...
    startStreamsSequentially()

    // Cleanup streams for devices no longer visible
    const visibleDeviceIds = new Set(authenticatedDevices.slice(0, getGridSize()).map(d => d.id))
